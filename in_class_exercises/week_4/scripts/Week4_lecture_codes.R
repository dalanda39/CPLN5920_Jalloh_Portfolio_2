...
# This code accesses the Census API key that is already stored in your R environment
...
# Check out the magic of the geometry column - a list within a column
...
# Simple plot
plot(pa_counties[1]) #the first column is the object ID
...
# Find counties that overlap core based statistical areas (any overlap)
# This code takes a little while to run
ggplot(metro_areas) +
  geom_sf() +
  theme_void()
...
head(county_summaries) 



# GEOMETRY OPERATIONS: CALCULATE DISTANCES + BUFFER ----------------------------
...
# NOTE: this is hypothetical, we don't have the type column! THIS CODE WILL RETURN AN ERROR
...
# GEOMETRY OPERATIONS: SPATIAL AGGREGATIONS ------------------------------------
...
# COORDINATE SYSTEMS: CALCULATE AREAS + EXAMINE PROJECTIONS --------------------

pa_counties <- pa_counties %>%
  mutate(
    area_sqkm = as.numeric(st_area(pa_counties)) / 1000000  #Returns an object of class "units." As numeric to convert to sq km
  )
...
ggplot(areas, aes(x = difference_pct)) +
  geom_histogram()
