...
#Now I am going to conver to the right crs 
...
st_write(crime_proj, "crime_proj.csv")
st_write(schools_proj, "schools_proj.csv")

st_write(crime_proj, "crime_project.shp")
st_write(schools_proj, "schools_project.shp")
...
#First I created a biffer of 1,000ft for my schools 
...
#Next I joined my crime data with the schools buffer, filtered for only the schools that were associated with the crime, grouped by the school name+ walkshed and then counted the total amount of crime per each school walkshed 
...
schools_buffer_bikes_plot<- schools_buffer_bikes_plot %>% 
  left_join(bike_lanes, by= "STREETNAME")

schools_buffer_bikes_plot<- st_as_sf(schools_buffer_bikes_plot, sf_column_name = "geometry")
